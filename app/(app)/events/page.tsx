...
function EventCard({
  ...
  return (
    <Card className={isPast ? "opacity-65" : undefined}>
      <CardHeader>
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            <CardTitle>{event.name}</CardTitle>
            <p className="mt-1 text-xs font-semibold uppercase tracking-wide text-red-700">{event.sampleLabel}</p>
          </div>
          <span
            className={`shrink-0 rounded-full px-2.5 py-1 text-xs font-semibold ${
              isPast
                ? "bg-white/[0.07] text-muted-foreground"
                : "border border-[rgba(242,181,44,0.3)] bg-[rgba(242,181,44,0.14)] text-amber-900"
            }`}
          >
      ...
      <CardContent className="grid gap-4">
        ...
        {(isJoined || needsBuddy || groupToShow) && (
          <div className="grid gap-2 rounded-xl border border-green-200 bg-green-50 p-3 text-sm text-green-800">
            {isJoined && (
              <p><span className="font-semibold">You joined this event.</span> It has been added to your event activity.</p>
            )}
            {needsBuddy && (
              <p><span className="font-semibold">Buddy request active.</span> Other students can see you on this event&apos;s Details page under Students seeking a buddy.</p>
            )}
            {groupToShow && (
              <p>
                ...
                {groupToShow.title} is listed on this event&apos;s Details page with {groupToShow.members.length}/{groupToShow.maxMembers} members.
              </p>
            )}
          </div>
        )}
...
export default function EventsPage() {
  ...
  // Resolved on the client so "already happened" follows the visitor's clock,
    ...
    <div className="depth-scene grid gap-6">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <h1 className="mt-1 text-3xl font-black text-navy">Events</h1>
          <p className="mt-2 max-w-2xl text-muted-foreground">
            Sample and community-added events across the semester, built so you can find someone to attend with.
          </p>
        </div>
