...
/**
 * Drives the 3D effects declared in globals.css:
 *
 * - any element with `.tilt-card` rotates toward the pointer and settles flat
 *   again when the pointer leaves
 * - any element with `.reveal-3d` swings into place the first time it scrolls
 *   into view
 *
 ...
 * Everything is skipped for coarse pointers and for reduced-motion users.
 */
export function TiltEffect() {
  useEffect(() => {
    ...
    function handlePointerMove(pointerEvent: PointerEvent) {
      ...
      const card = target?.closest<HTMLElement>(".tilt-card") ?? null;
