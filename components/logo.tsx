...
export function Logo({ href = "/", className, compact = false }: LogoProps) {
  return (
    <Link href={href} className={cn("focus-ring group inline-flex items-center gap-3 rounded-2xl", className)}>
      <span className="logo-mark relative grid size-11 place-items-center overflow-hidden rounded-2xl border border-white/10 bg-[#140a0d] text-sm font-black text-white shadow-glow">
